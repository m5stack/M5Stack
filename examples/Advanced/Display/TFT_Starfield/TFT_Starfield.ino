...
void setup() {
    ...
    // It must be called after any other graphics drawing function call if
    // fastPixel() is to be called again
...
void loop() {
    ...
    for (int i = 0; i < NSTARS; ++i) {
        if (sz[i] <= 1) {
            ...
            // This is a faster pixel drawing function for occassions where many
            // single pixels must be drawn
            ...
            if (sz[i] > 1) {
                ...
                if (screen_x >= 0 && screen_y >= 0 && screen_x < 320 &&
                    screen_y < 240) {
                    uint8_t r, g, b;
                    r = g = b = 255 - sz[i];
