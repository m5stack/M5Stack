...
void setup() {
    ...
    // The scroll area is set to the full sprite size upon creation of the
    // sprite but we can change that by defining a smaller area using
    // "setScrollRect()"if needed parameters are x,y,w,h,color as in drawRect(),
    // the color fills the gap left by scrolling
    // graph1.setScrollRect(64, 0, 64, 61, TFT_DARKGREY);  // Try this line to
    // change the graph scroll area
    ...
    stext1.setScrollRect(
        0, 0, 32, 64, TFT_BLUE);  // here we set scroll gap fill color to blue
    ...
    stext2.setScrollRect(
        0, 0, 40, 16,
        TFT_DARKGREY);  // Scroll the "Hello" in the first 40 pixels
...
void loop() {
    ...
    // later)
    graph1.drawFastVLine(127, 60 - graphVal, 2,
                         TFT_YELLOW);  // draw 2 pixel point on graph
    ...
    if (tcount <= 0) {  // If we have scrolled 40 pixels the redraw text
        tcount = 40;
        stext2.drawString("Hello World", 6, 0,
                          2);  // draw at 6,0 in sprite, font 2
    }
