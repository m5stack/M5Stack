...
void loop() {
    if (targetTime < millis()) {
        ...
        M5.Lcd.setTextColor(
            TFT_BLACK);  // Text background is not defined so it is transparent
        ...
        M5.Lcd.drawString(
            "Font size 2", xpos, 14,
            2);  // Draw text centre at position xpos, 14 using font 2
        M5.Lcd.drawString(
            "Font size 4", xpos, 30,
            4);  // Draw text centre at position xpos, 30 using font 4
        M5.Lcd.drawString(
            "12.34", xpos, 54,
            6);  // Draw text centre at position xpos, 54 using font 6

        M5.Lcd.drawString(
            "12.34 is in font size 6", xpos, 92,
            2);  // Draw text centre at position xpos, 92 using font 2
        ...
        M5.Lcd.setTextDatum(TR_DATUM);  // Top Right datum so text butts neatly
                                        // to xpos (right justified)

        M5.Lcd.drawFloat(pi, precision, xpos, ypos,
                         2);  // Draw rounded number and return new xpos delta
                              // for next print position

        M5.Lcd.setTextDatum(TL_DATUM);  // Top Left datum so text butts neatly
                                        // to xpos (left justified)
        ...
        M5.Lcd.setTextFont(2);  // Select font 2, now we do not need to specify
                                // the font in drawString()

        // An easier way to position text and blank old text is to set the datum
        // and use width padding
        M5.Lcd.setTextDatum(BC_DATUM);  // Bottom centre for text datum
        M5.Lcd.setTextPadding(M5.Lcd.width() +
                              1);  // Pad text to full screen width + 1 spare
                                   // for +/-1 position rounding
        ...
        // Now call the screen server to send a copy of the TFT screen to the PC
        // running the Processing client sketch
        screenServer();
    }
}
