...
bool Voltmeter::i2cReadBytes(uint8_t addr, uint8_t reg_addr, uint8_t* buff,
                             uint16_t len) {
...
bool Voltmeter::i2cWriteBytes(uint8_t addr, uint8_t reg_addr, uint8_t* buff,
                              uint16_t len) {
...
bool Voltmeter::saveCalibration2EEPROM(voltmeterGain_t gain, int16_t hope,
                                       int16_t actual) {
    if (hope == 0 || actual == 0) {
        return false;
    }
...
bool Voltmeter::readCalibrationFromEEPROM(voltmeterGain_t gain, int16_t* hope,
                                          int16_t* actual) {
    uint8_t addr = getPGAEEEPROMAddr(gain);
    uint8_t buff[8];
    memset(buff, 0, 8);
