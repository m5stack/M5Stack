...
void setup() {
    ...
    M5.Lcd.setCursor(80, 0,
                     4);  // Set the cursor position to (80,0),and set it to a 4
                          // point font.  将光标位置设置为(80,0),设置为4号字体
...
void loop() {
    ...
    if (M5.BtnC.wasPressed()) {
        ...
        if (all_flag) {
            ...
            if (sync_flag)
                unit_4relay.relayALL(
                    1);  // Open all the relays.  打开所有的继电器
            else
            ...
            if (sync_flag)
                ...
                unit_4relay.LED_ALL(
                    0);  // Turn off all the lights.  关闭所有的灯
        }
    }
}