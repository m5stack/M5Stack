...
unsigned long testRects(uint16_t color)
{
  ...
  M5.Lcd.fillScreen(TFT_BLACK);
  n = (M5.Lcd.width() > M5.Lcd.height()) ? M5.Lcd.height() : M5.Lcd.width();
  start = micros();
  for (i = 2; i < n; i += 6)
  {
    i2 = i / 2;
...
unsigned long testRoundRects()
{
  ...
  M5.Lcd.fillScreen(TFT_BLACK);
  w = (M5.Lcd.width() > M5.Lcd.height()) ? M5.Lcd.height() : M5.Lcd.width();
  start = micros();
  for (i = 0; i < w; i += 6)
  {
    i2 = i / 2;
...
unsigned long testFilledRoundRects()
{
  ...
  M5.Lcd.fillScreen(TFT_BLACK);
  start = micros();
  for (i = (M5.Lcd.width() > M5.Lcd.height()) ? M5.Lcd.height() : M5.Lcd.width(); i > 20; i -= 6)
  {
    i2 = i / 2;
